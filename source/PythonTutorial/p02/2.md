...
元组（英文 `tuple`），和列表非常的相似，它也是由一系列元素按顺序进行排列而成的容器。
...
**第三种方法**：使用元组推导式，由于元组是不可变的，所以生成一个生成器对象。这一种对于新手来说可能会比较难以理解，我会放在后面专门进行讲解，这里先作了解。

```python
>>> ctuple = ( i for i in range(1,6) )
>>> ctuple
<generator object <genexpr> at 0x00000122AEAB6430>
```

不过值得注意的是，当你在创建只有一个元素的元组时，你有可能会这样子创建
...
前面我们说过，元组是不可变的。因此，你想对元组进行修改的行为都是不被允许的。

看如下示例，查看元素可以，但是修改元素和删除元素都会报错。
