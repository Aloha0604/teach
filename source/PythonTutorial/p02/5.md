...
譬如我们前面学过的 列表、元组、字典、字符串 等都是可迭代对象。
...
但是对新手来说，可能需要借助一些函数来判别，比如 Python 内置的 `collections.abc` 模块，

这个模块只有在 Python 中才有，在这个模块中提供了一个 `Iterable类`，可以用 `isinstance()` 来判断。
...
True
>>> isinstance({"name": "WillWang"}, Iterable) # 字典
True
